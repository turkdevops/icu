...
public final class Utility {
     ...
     */
    public static final boolean arrayRegionMatches(
            Object[] source, int sourceStart, Object[] target, int targetStart, int len) {
        int sourceEnd = sourceStart + len;
        int delta = targetStart - sourceStart;
        for (int i = sourceStart; i < sourceEnd; i++) {
            if (!arrayEquals(source[i], target[i + delta])) return false;
        }
        return true;
    }
     ...
     *
     * @param len the length to compare. The start indices and start+len must be valid. Ought to be
     *     in System
     */
    public static final boolean arrayRegionMatches(
            int[] source, int sourceStart, int[] target, int targetStart, int len) {
        int sourceEnd = sourceStart + len;
        int delta = targetStart - sourceStart;
        for (int i = sourceStart; i < sourceEnd; i++) {
            if (source[i] != target[i + delta]) return false;
     ...
     */
    public static final boolean arrayRegionMatches(
            double[] source, int sourceStart, double[] target, int targetStart, int len) {
        int sourceEnd = sourceStart + len;
        int delta = targetStart - sourceStart;
        for (int i = sourceStart; i < sourceEnd; i++) {
            if (source[i] != target[i + delta]) return false;
    ...
    /**
     * Trivial reference equality. This method should help document that we really want == not
     * equals(), and to have a single place to suppress warnings from static analysis tools.
     */
    ...
    /**
     * The ESCAPE_BYTE character is used during run-length encoding. It signals a run of identical
     * bytes.
     */
    static final byte ESCAPE_BYTE = (byte) 0xA5;

    /**
     * Construct a string representing an int array. Use run-length encoding. A character represents
     * itself, unless it is the ESCAPE character. Then the following notations are possible: ESCAPE
     * ESCAPE ESCAPE literal ESCAPE n c n instances of character c Since an encoded run occupies 3
     * characters, we only encode runs of 4 or more characters. Thus we have n > 0 and n != ESCAPE
     * and n <= 0xFFFF. If we encounter a run where n == ESCAPE, we represent this as: c ESCAPE n-1
     * c The ESCAPE value is chosen so as not to collide with commonly seen values.
     */
    ...
    /**
     * Construct a string representing a short array. Use run-length encoding. A character
     * represents itself, unless it is the ESCAPE character. Then the following notations are
     * possible: ESCAPE ESCAPE ESCAPE literal ESCAPE n c n instances of character c Since an encoded
     * run occupies 3 characters, we only encode runs of 4 or more characters. Thus we have n > 0
     * and n != ESCAPE and n <= 0xFFFF. If we encounter a run where n == ESCAPE, we represent this
     * as: c ESCAPE n-1 c The ESCAPE value is chosen so as not to collide with commonly seen values.
     */
    ...
    /**
     * Construct a string representing a char array. Use run-length encoding. A character represents
     * itself, unless it is the ESCAPE character. Then the following notations are possible: ESCAPE
     * ESCAPE ESCAPE literal ESCAPE n c n instances of character c Since an encoded run occupies 3
     * characters, we only encode runs of 4 or more characters. Thus we have n > 0 and n != ESCAPE
     * and n <= 0xFFFF. If we encounter a run where n == ESCAPE, we represent this as: c ESCAPE n-1
    ...
    /**
     * Construct a string representing a byte array. Use run-length encoding. Two bytes are packed
     * into a single char, with a single extra zero byte at the end if needed. A byte represents
     * itself, unless it is the ESCAPE_BYTE. Then the following notations are possible: ESCAPE_BYTE
     * ESCAPE_BYTE ESCAPE_BYTE literal ESCAPE_BYTE n b n instances of byte b Since an encoded run
     * occupies 3 bytes, we only encode runs of 4 or more bytes. Thus we have n > 0 and n !=
     * ESCAPE_BYTE and n <= 0xFF. If we encounter a run where n == ESCAPE_BYTE, we represent this
     * as: b ESCAPE_BYTE n-1 b The ESCAPE_BYTE value is chosen so as not to collide with commonly
     * seen values.
     */
     ...
     */
    private static final <T extends Appendable> void encodeRun(
            T buffer, byte value, int length, byte[] state) {
        if (length < 4) {
            for (int j = 0; j < length; ++j) {
    ...
    /**
     * Append a byte to the given Appendable, packing two bytes into each character. The state
     * parameter maintains intermediary data between calls.
     *
     * @param state A two-element array, with state[0] == 0 if this is the first byte of a pair, or
     *     state[0] != 0 if this is the second byte of a pair, in which case state[1] is the first
     *     byte.
     */
    private static final <T extends Appendable> void appendEncodedByte(
            T buffer, byte value, byte[] state) {
        try {
            if (state[0] != 0) {
    ...
    public static final short[] RLEStringToShortArray(String s) {
        ...
        if (ai != length) throw new IllegalStateException("Bad run-length encoded short array");
    ...
    public static final byte[] RLEStringToByteArray(String s) {
        ...
        if (node != 0) throw new IllegalStateException("Bad run-length encoded byte array");
    ...
    /**
     * Format a String for representation in a source file. This includes breaking it into lines and
     * escaping characters using octal notation when necessary (control characters and double
     * quotes).
     */
    ...
    /**
     * Format a String for representation in a source file. Like formatForSource but does not do
     * line breaking.
     */
     ...
     * backslash to a double backslash.
     */
    ...
    /**
     * Converts an escape to a code point value. We attempt to parallel the icu4c unescapeAt()
     * function. This function returns an integer with both the code point (bits 28..8) and the
     * length of the escape sequence (bits 7..0). offset+length is the index after the escape
     * sequence.
     *
     ...
     *
     * @exception IllegalArgumentException if an invalid escape is seen.
     */
    ...
    /**
     * Convert all escapes in a given string using unescapeAndLengthAt(). Leave invalid escape
     * sequences unchanged.
     */
    ...
    /**
     * Convert a string to comma-separated groups of 4 hex uppercase digits. E.g., hex('ab') =>
     * "0041,0042".
     */
    ...
    /**
     * Convert a string to separated groups of hex uppercase digits. E.g., hex('ab'...) =>
     * "0041,0042". Append the output to the given Appendable.
     */
    public static <S extends CharSequence, U extends CharSequence, T extends Appendable> T hex(
            S s, int width, U separator, boolean useCodePoints, T result) {
        try {
            if (useCodePoints) {
                int cp;
     ...
     * @param divider the character on which to split. Occurrences of this character are not
     *     included in the output
     * @param output an array to receive the substrings between instances of divider. It must be
     *     large enough on entry to accommodate all output. Adjacent instances of the divider
     *     character will place empty strings into output. Before returning, output is padded out
     *     with empty strings.
     */
     ...
     * @param divider the character on which to split. Occurrences of this character are not
     *     included in the output
     * @return output an array to receive the substrings between instances of divider. Adjacent
     *     instances of the divider character will place empty strings into output.
     */
    ...
    /**
     * Look up a given string in a string array. Returns the index at which the first occurrence of
     * the string was found in the array, or -1 if it was not found.
     *
     * @param source the string to search for
     * @param target the array of zero or more strings in which to look for source
     * @return the index of target at which source first occurs, or -1 if not found
     */
     ...
     * @param pos INPUT-OUTPUT parameter. On input, pos[0] is the offset of the first character to
     *     be parsed. On output, pos[0] is the index after the last parsed character. If the parse
     *     fails, pos[0] will be unchanged.
     * @param ch the non-whitespace character to be parsed.
     * @return true if 'ch' is seen preceded by zero or more whitespace characters.
     */
    ...
    /**
     * Parse a pattern string starting at offset pos. Keywords are matched case-insensitively.
     * Spaces may be skipped and may be optional or required. Integer values may be parsed, and if
     * they are, they will be returned in the given array. If successful, the offset of the next
     * non-space character is returned. On failure, -1 is returned.
     *
     * @param pattern must only contain lowercase characters, which will match their uppercase
     *     equivalents as well. A space character matches one or more required spaces. A '~'
     *     character matches zero or more optional spaces. A '#' character matches an integer and
     *     stores it in parsedInts, which the caller must ensure has enough capacity.
     * @param parsedInts array to receive parsed integers. Caller must ensure that parsedInts.length
     *     is >= the number of '#' signs in 'pattern'.
     * @return the position after the last character parsed, or -1 if the parse failed
     */
    @SuppressWarnings("fallthrough")
    public static int parsePattern(
            String rule, int pos, int limit, String pattern, int[] parsedInts) {
    ...
    /**
     * Parse a pattern string within the given Replaceable and a parsing pattern. Characters are
     * matched literally and case-sensitively except for the following special characters:
     *
     * <p>~ zero or more Pattern_White_Space chars
     *
     * <p>If end of pattern is reached with all matches along the way, pos is advanced to the first
     * unparsed index and returned. Otherwise -1 is returned.
     *
     ...
     * that is, in standard decimal, hex, or octal format.
     *
     * @param pos INPUT-OUTPUT parameter. On input, the first character to parse. On output, the
    ...
    /**
     * Parse a Unicode identifier from the given string at the given position. Return the
     * identifier, or null if there is no identifier.
     *
     * @param str the string to parse
     * @param pos INPUT-OUTPUT parameter. On INPUT, pos[0] is the first character to examine. It
     *     must be less than str.length(), and it must not point to a whitespace character. That is,
     *     must have pos[0] < str.length(). On OUTPUT, the position after the last parsed character.
     * @return the Unicode identifier, or null if there is no valid identifier at pos[0].
     */
    ...
    /**
     * Append the digits of a positive integer to the given <code>Appendable</code> in the given
     * radix. This is done recursively since it is easiest to generate the low- order digit first,
     * but it must be appended last.
     *
     ...
     */
    private static <T extends Appendable> void recursiveAppendNumber(
            T result, int n, int radix, int minDigits) {
        try {
            int digit = n % radix;
    ...
    /**
     * Append a number to the given Appendable in the given radix. Standard digits '0'-'9' are used
     * and letters 'A'-'Z' for radices 11 through 36.
     *
     * @param result the digits of the number are appended here
     * @param n the number to be converted to digits; may be negative. If negative, a '-' is
     *     prepended to the digits.
     * @param radix a radix from 2 to 36 inclusive.
     * @param minDigits the minimum number of digits, not including any '-', to produce. Values less
     *     than 2 have no effect. One digit is always emitted regardless of this parameter.
    ...
    /**
     * Parse an unsigned 31-bit integer at the given offset. Use UCharacter.digit() to parse
     * individual characters into digits.
     *
     * @param text the text to be parsed
     * @param pos INPUT-OUTPUT parameter. On entry, pos[0] is the offset within text at which to
     *     start parsing; it should point to a valid digit. On exit, pos[0] is the offset after the
     *     last parsed character. If the parse failed, it will be unchanged on exit. Must be >= 0 on
     *     entry.
     * @param radix the radix in which to parse; must be >= 2 and <= 36.
     * @return a non-negative parsed number, or -1 upon parse failure. Parse fails if there are no
     *     digits, that is, if pos[0] does not point to a valid digit on entry, or if the number to
     *     be parsed does not fit into a 31-bit unsigned integer.
     */
    ...
    /**
     * Escapes one unprintable code point using <backslash>uxxxx notation for U+0000 to U+FFFF and
    ...
    /**
     * Escapes one code point using <backslash>uxxxx notation for U+0000 to U+FFFF and
     * <backslash>Uxxxxxxxx for U+10000 and above.
     *
     * @return result
     */
    ...
    /**
     * Returns the index of the first character in a set, ignoring quoted text. For example, in the
     * string "abc'hide'h", the 'h' in "hide" will not be found by a search for "h". Unlike
     * String.indexOf(), this method searches not for a single character, but for any character of
     * the string <code>setOfChars</code>.
     *
     ...
     * @return Offset of the first character in <code>setOfChars</code> found, or -1 if not found.
     * @see String#indexOf
     */
    ...
    /**
     * Append a character to a rule that is being built up. To flush the quoteBuf to rule, make one
     * final call with isLiteral == true. If there is no final character, pass in (int)-1 as c.
     *
     * @param rule the string to append the character to
     * @param c the character to append, or (int)-1 if none.
     * @param isLiteral if true, then the given character should not be quoted or escaped. Usually
     *     this means it is a syntactic element such as > or $
     * @param escapeUnprintable if true, then unprintable characters should be escaped using
     *     escapeUnprintable(). These escapes will appear outside of quotes.
     * @param quoteBuf a buffer which is used to build up quoted substrings. The caller should
     *     initially supply an empty buffer, and thereafter should not modify the buffer. The buffer
     *     should be cleared out by, at the end, calling this method with a literal character (which
     *     may be -1).
     */
     ...
     * @return 0 if equals, 1 if source is greater than target and -1 otherwise
     */
    ...
    /**
     * Find the highest bit in a positive integer. This is done by doing a binary search through the
     * bits.
     *
     * @param n is the integer
     * @return the bit number of the highest bit, with 0 being the low order bit, or -1 if <code>n
     *     </code> is not positive
     */
    ...
    /**
     * Utility method to take a int[] containing codepoints and return a string representation with
     * code units.
     */
    ...
    /**
     * Java 8+ String#join(CharSequence, Iterable<? extends CharSequence>) compatible method for
     * Java 7 env.
     *
